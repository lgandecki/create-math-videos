...
class IllustrateExponentialGrowth(Scene):
    ...
    def construct(self):
        ...
        self.play(self.camera.frame.animate.move_to(squares[19].get_center()).set(width=board.width / 4))
        
        pile_20 = Circle(radius=1.5, color=WHEAT_COLOR, fill_opacity=0.8).move_to(squares[19].get_center())
        label_20 = Text("Over 500,000 grains!", font_size=32, color=TEXT_COLOR).next_to(pile_20, UP)
        self.play(GrowFromCenter(pile_20), Write(label_20))
        self.wait(2)
        
        # --- Acceleration and Massive Scale ---
        self.play(self.camera.frame.animate.move_to(ORIGIN).set(width=board.width * 1.5))
        self.wait(0.5)

        # Square 30: Warehouse
        label_30 = Text("Square 30: > 500 Million Grains!", font_size=36, color=TEXT_COLOR).to_edge(UP).shift(DOWN*1.5)
        warehouse = Rectangle(height=2, width=3.5, color=WHEAT_COLOR, fill_opacity=0.8).next_to(squares[29], RIGHT, buff=1)
        self.play(ReplacementTransform(pile_20, warehouse), ReplacementTransform(label_20, label_30), run_time=2)
        self.wait(2)
        
        # Square 40: Mountain
        label_40 = Text("Square 40: > 500 Billion Grains!", font_size=36, color=TEXT_COLOR).to_edge(UP).shift(DOWN*1.5)
        mountain = Polygon(
            [-4, -1, 0], [0, 2, 0], [4, -1, 0],
            color=WHEAT_COLOR, fill_opacity=0.8
        ).shift(DOWN * 0.5)
        self.play(ReplacementTransform(warehouse, mountain), ReplacementTransform(label_30, label_40), run_time=2)
        self.wait(2)

        # --- The Lesson ---
        self.play(FadeOut(board, mountain), FadeOut(*self.mobjects[-8:])) # Clear board and grains
        self.wait(1)
        
        conclusion_title = Text("This is Exponential Growth.", font_size=42, color=ACCENT_COLOR_1).to_edge(UP)
        conclusion_line1 = Text("Deceptively small at first...", font_size=36, color=TEXT_COLOR).next_to(conclusion_title, DOWN, buff=0.7)
        conclusion_line2 = Text("...then shockingly enormous.", font_size=36, color=TEXT_COLOR).next_to(conclusion_line1, DOWN, buff=0.3)
        ...
        self.play(FadeOut(conclusion_title, conclusion_line1, conclusion_line2))
        
        # --- Final Question ---
        final_question = Text(
            "Where else might exponential growth\nbe quietly at work in your life?",
            font_size=40,
            color=TEXT_COLOR,
            line_spacing=1.5,
            t2c={"exponential growth": ACCENT_COLOR_2}
        ).center()
        
        self.play(Write(final_question))
        
        # Cleanup for scene combination
        self.mobjects = [title, board, *self.mobjects[-17:]]
